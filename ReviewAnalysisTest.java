...
public class ReviewAnalysisTest {
	...
	public void testConvertFileToStringArray() {
		String[] lines = ReviewAnalysis.convertFileToStringArray("MovieReviews.txt");
		assertEquals("failed to add lines to array", 8529, lines.length);
		assertEquals("failed to add make all text lower case", "1 a series of escapades demonstrating the adage that what is good for the goose is also good for the gander , some of which occasionally amuses but none of which amounts to much of a story .", lines[0]);
	}
