...
public class ReviewAnalysis {
	...
	public static double scoreReview(String review, String[] words, double[] scores, int[] wordCount, int numberOfWordsSoFar) {
		...
		while (s.hasNext()) {
		    token = s.next();
		    for (int index = 0; index < numberOfWordsSoFar; index++ ) {
		        if (token.equals(words[index])) {
		            average += scores[index]/wordCount[index];
		        }
		    }
		}
		/// try to call for indexOfWordsInArray later
		/*while (s.hasNext()) {
		    index = indexOfWordInArray(words, review, numberOfWordsSoFar);
		}*/
		return average/numberOfWordsSoFar;
	}
	...
	public static int indexOfBestWord(double[] scores, int[] counts, int countAbove, int numberOfWordsSoFar) {
		double highestScores = 0.0;
		double score;
		int Index = -1;
	    for (int i=0; i<numberOfWordsSoFar; i++) {
		    if (counts[i] > countAbove) {
		        score = scores[i]/(double) counts[i];
		        if (highestScores < score) {
		            highestScores = score;
		            Index = i;
		        }
		    }
	...
	public static int processWords(Scanner s, int lineScore, String[] words, double[] word_score, int[] word_count, int numberOfWordsSoFar) {
		...
		while (s.hasNext()){
		    ...
		    if (words[0] == null) {
		        ...
		        numberOfWordsSoFar++;
		        word_count[0] += 1;
		        word_score[0] += lineScore;
		    }
		    int n = 0;
		    int a = 0;
		    while (n != numberOfWordsSoFar) {
		        for (int j = 0; j < numberOfWordsSoFar; j++) {
        		        if (!token.equals(words[j])) {
        		            a++;
        		        }
		        }
		        if (a < numberOfWordsSoFar){
		            for (int b = 0; b < numberOfWordsSoFar; b++) {
		                if (token.equals(words[b])) {
		                    word_score[b] += lineScore;
		                    word_count[b] += 1;
		                }
		            }
		            numberOfWordsSoFar--;
		        }
		        else if (a == numberOfWordsSoFar) {
		            words[numberOfWordsSoFar] = token;
		            word_count[a] += 1;
		            word_score[a] += lineScore;
		        }
		        i++;
		        n++;
		    }
	...
	public static String[] convertFileToStringArray(String filename) {
	    String[] wholeText = new String[8529];
        int i = 0;
        try {
          File file = new File(filename);
          Scanner fs = new Scanner(file);
          while(fs.hasNextLine()) {
              String singleLine = fs.nextLine();
              String lowerCase = singleLine.toLowerCase();
              wholeText[i] = lowerCase;
              i++;
          }
          fs.close();
      }
